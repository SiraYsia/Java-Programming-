...
public class SearchTreeMap<K extends Comparable<K>, V> {
	...
	/*
	 * This method returns list of keys in map in natural sorted order
	 * 
	 */
	...
	/*
	 * This method returns subset of TreeMap between the values fromKey-toKey. It
	 * will include fromKey and toKey if they are found in the original map. The
	 * values for fromKey and toKey do not actually need to be in the map. You can
	 * assume than fromKey is less than or equal to toKey.
	 * 
	 */
	public SearchTreeMap<K, V> subMap(K fromKey, K toKey) {
		SearchTreeMap<K, V> subTree = new SearchTreeMap<K, V>();
		subTree.theTree = theTree.subTree(fromKey, toKey);
		return subTree;
	}
}